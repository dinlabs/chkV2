...
class UserController extends ResourceController
{
    ...
    protected function prepareResetPasswordRequest(Request $request, GeneratorInterface $generator, string $senderEvent): Response
    {
        ...
        if (in_array($request->getMethod(), ['POST', 'PUT', 'PATCH'], true) && $form->handleRequest($request)->isValid()) {
            ...
            /* ajout Yannick */
            // if user doesn't exists on website...
            if(is_null($user))
            {
                $emailForm = $request->get('sylius_user_request_password_reset');
                $email = $emailForm['email'];
                
                $ginkoiaCustomerWs = $this->get('app.ginkoiaws');
                if(($_user = $ginkoiaCustomerWs->getCustomerInfos($email)) && isset($_user['ID']))
                {
